...
static bool8 LoadBattlerSpriteGfx(u8 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            LoadPalette(gTrainerBackPicPaletteTable[TRAINER_BACK_PIC_POKE_DUDE].data, 0x100 + 16 * battler, 32); 
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            SetMultiuseSpriteTemplateToPokemon(GetFormSpecies(GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], MON_DATA_SPECIES), GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], MON_DATA_FORM)), GetBattlerPosition(battler));
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(TRAINER_BACK_PIC_POKE_DUDE, GetBattlerPosition(0));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBackPicCoords[TRAINER_BACK_PIC_POKE_DUDE].size) * 4 + 80,
                                                 GetBattlerSpriteSubpriority(0));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = battler;
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
